...
const ProductList = ({route,navigation}) => {
   ...
   const [ loading,setLoading]=useState(false)
    ...
    const addtocart = async(item,minus) => {
      ...
      const body={
          ...
          listId:listid,
          price:item?.price,
          qty:find?(minus?find?.value -1:find?.value+1):1
      }
      setLoading(true)
      http.get('/', {
          params: {
      ...
      const renderitem2 = ({item}) => {
        ...
        <Text style={[globalStyles.text2,{color:theme.colors.primary}]}>₹ {item?.price}</Text>
     {loading?<ActivityIndicator/>:(selectedList?.find((it)=>it?.productId==item?.productId)?
               
               <View style={[globalStyles.rowflex,{width:50}]}>
               <TouchableOpacity onPress={()=>addtocart(item,"minues")} style={[styels.coutnerButton]}><Text style={[globalStyles.text,{fontSize:15}]}>-</Text></TouchableOpacity>
               <Text style={[globalStyles.text]}>{selectedList?.find((it)=>it?.productId==item?.productId)?.value}</Text>
               <TouchableOpacity onPress={()=>addtocart(item)} style={[styels.coutnerButton]}><Text style={[globalStyles.text,{fontSize:15}]}>+</Text></TouchableOpacity>

               </View>:<TouchableOpacity onPress={()=>addtocart(item)} style={{backgroundColor:"black",paddingHorizontal:5,borderRadius:5,marginTop:10}}>
  ...
  return (
    <View style={[globalStyles.container2]}>
      ...
      />
                  {selectedList?.length>0&&<CustomButton loading={loading} onPressfuntion={()=>{navigate("ViewCart",{listid:listid})}} text={"View Cart"} marginTop={"auto"} />}
